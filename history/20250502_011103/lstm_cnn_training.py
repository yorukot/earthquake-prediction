...
# Parameters
sequence_length = 7  # Reduced from 14 to 7 days of data to predict the next day
...
print(f"Preparing to predict for {target_date}")
print(f"Using data from {seq_start_date} to {seq_end_date}")
...
if len(pred_seq_df) > 0:
    ...
    print(f"Daily prediction data:\n{pred_daily_data[['date']].values}")
    ...
    if len(pred_daily_data) == sequence_length:
        ...
        print("Using available data anyway for demonstration purposes...")
        ...
        print(f"Prediction for {target_date.date()} (with padding):")
        print(f"Probability of earthquake: {earthquake_probability:.2f}%")
        print(f"Prediction: {'Earthquake' if prediction > 0.5 else 'No Earthquake'}")
