...
for day in pd.date_range('2024-08-01', '2024-08-31'):
    ...
    has_eq = len(y_pred_day) > 0  # 只要有預測值就代表有地震
    max_pred = np.max(y_pred_day) if has_eq else '無資料'
    print(f'{day.strftime("%Y-%m-%d")} 預測有地震嗎？ {"有" if has_eq else "沒有"}，最大預測震度: {max_pred}')
