...
# 訓練集：2024-04-01 之前
train_df = df[df['time'] < '2024-04-01']

# 測試集：2024-04-01 ~ 2024-04-30
test_df = df[(df['time'] >= '2024-04-01') & (df['time'] < '2024-05-01')]
...
# 檢查是否有預測值大於 6
has_big_eq = np.any(y_pred > 6)
print(f'2024年4月模型預測有大於6級地震嗎？ {"有" if has_big_eq else "沒有"}')
print('4月預測最大震度:', np.max(y_pred) if len(y_pred) > 0 else '無資料')
